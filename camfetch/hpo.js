...
function HPO_nonrepeating(termRef,codeRef){
	
	if(typeof(termRef) == 'undefined') return;
	
		var termField = jQuery("#"+termRef).parent().parent().find("input");
		
		termField.on("keyup",termField, function() {
			termField.attr('autocomplete', 'off');
			connectToHPOServiceNonRep(termRef, codeRef);
			...
			termF.autocomplete(
					{
						...
						minLength : 3,
						select : function(event, ui) {
							jQuery("#"+termField).val(ui.item.name);
							
							if(typeof(codeField) !== 'undefined'){
								codeF = jQuery("#"+codeField).parent().parent().find("input");
								codeF.val(ui.item.id);						
							}
